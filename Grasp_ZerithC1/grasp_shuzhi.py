...
# ---- 工作区掩码与相机内参 (保持不变) ----
WORKSPACE_MASK_PATH = "/home/wmx/graspnet-baseline/mask.png"
...
xunshi_pos = np.array([ 0.5364 ,-0.2829 , 0.1407])#卫生间右侧巡视位姿
xunshi_euler_deg =  np.array([-175.4348 ,   2.1362 , 171.5162])#垃圾桶位置



binstart_pos =np.array( [ 0.5167 ,0.2731, -0.0299])
binstart_euler_deg = np.array([-1.799356e+02 ,-3.470000e-02 ,-1.759093e+02] )###卫生间内侧垃圾的抓取起始位姿
binstart_pose = Pose.from_xyz_rpy(binstart_pos, binstart_euler_deg)

binstart_pos_up =np.array( [ 0.5167 ,0.2731, 0.1])
binstart_euler_deg_up = np.array([-1.799356e+02 ,-3.470000e-02 ,-1.759093e+02] )###卫生间内侧垃圾的抓取起始位姿
binstart_pose_up = Pose.from_xyz_rpy(binstart_pos_up, binstart_euler_deg_up)


bin_pos = np.array([ 0.8506 , 0.2717 ,-0.0651])

bin_euler_deg =np.array( [ 178.7773  , 12.6751 ,-167.8946]) #垃圾桶位置

bin_pose = Pose.from_xyz_rpy(bin_pos, bin_euler_deg)

bin_pos_up = np.array([ 0.8506 , 0.2717 ,0.04])

bin_euler_deg_up =np.array( [ 178.7773  , 12.6751 ,-167.8946]) #垃圾桶位置
bin_pose_up = Pose.from_xyz_rpy(bin_pos_up, bin_euler_deg_up)
# # ---- 手眼标定与工具TCP配置  ----
...
def main():
    ...
    try:
        while True:
            # --- 2. 感知 ---
            input("\n按回车键开始新一轮抓取检测...")
            robot.MoveL(binstart_pos_up, binstart_euler_deg_up, speed=ROBOT_SPEED, acc=ROBOT_ACC)
            robot.MoveL(binstart_pos, binstart_euler_deg, speed=ROBOT_SPEED, acc=ROBOT_ACC)
            ...
            goal_pose.rpy_deg = np.asarray(current_euler_deg, dtype=np.float64)
            ...
            if confirm.lower() == 'y':
                ...
                print(f"  > 正在前往垃圾桶...")
                trajectory_planner.execute_cartesian_trajectory(
                    robot=robot, start_pose=binstart_pose_up, goal_pose=bin_pose, planner_name="sqp",
                    speed=0.2, acc=0.1, duration=10.0, num_samples=5, zoneRadius="Z100")
                # print("\n  ✓ 抓取序列执行完毕！")
                # trajectory_planner.execute_cartesian_trajectory(
                #     robot=robot, start_pose=bin_pose_up, goal_pose=bin_pose, planner_name="sqp",
                #     speed=0.2, acc=0.1, duration=10.0, num_samples=5, zoneRadius="Z100")
                ...
                trajectory_planner.execute_cartesian_trajectory(
                    robot=robot, start_pose=bin_pose, goal_pose=binstart_pose, planner_name="sqp",
                    speed=0.2, acc=0.1, duration=10.0, num_samples=5, zoneRadius="Z100")
                # print(f"  > 正在回到初始位置...")
                # trajectory_planner.execute_cartesian_trajectory(
