...
binstart_euler_deg = np.array([-1.799356e+02, -3.470000e-02, -1.759093e+02])
binstart_pose = Pose.from_xyz_rpy(binstart_pos.tolist(), binstart_euler_deg.tolist())
...
def perform_complete_grasp_cycle(net, robot, trajectory_planner, workspace_mask_path):
    ...
    if confirm.lower() == 'y':
        ...
        trajectory_planner.execute_cartesian_trajectory(
            robot=robot, start_pose=binstart_pose, goal_pose=goal_pose,
        ...
        print("  > 正在抬升物体...")
        trajectory_planner.execute_cartesian_trajectory(
            robot=robot, start_pose=goal_pose, goal_pose=binstart_pose_up,
            planner_name="s_curve", speed=0.2, acc=0.1, duration=10.0, num_samples=5
        )
        
        print("  > 正在前往垃圾桶...")
        trajectory_planner.execute_cartesian_trajectory(
            robot=robot, start_pose=binstart_pose_up, goal_pose=bin_pose,
        ...
        print("  > 正在返回抓取起始点...")
        trajectory_planner.execute_cartesian_trajectory(
            robot=robot, start_pose=bin_pose, goal_pose=binstart_pose,
            planner_name="sqp", speed=0.2, acc=0.1, duration=10.0, num_samples=5
        )
...
def Grasp_api(workspace_mask_path):
    ...
    try:
        while True:
            ...
            robot.MoveL(binstart_pos.tolist(), binstart_euler_deg.tolist(), speed=ROBOT_SPEED, acc=ROBOT_ACC)
